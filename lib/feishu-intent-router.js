...
const INTENT_CATALOG = [
  ...
  { id: 'anniversaries', desc: '近期纪念日、有哪些纪念日' },
  { id: 'person_query', desc: '查某个人的生日/纪念日/相关事项（如：小明的生日、妈妈的纪念日）' },
  { id: 'upcoming', desc: '近期日程、即将到来的提醒、这周有什么' },
  { id: 'inventory', desc: '全部清单、有哪些习惯/日子/待办' },
  { id: 'summary', desc: '项目概况、整体状态、总结一下' },
  { id: 'search_by_tag', desc: '按标签查事项（如：查标签健康、前端有什么事）' },
  { id: 'list_tags', desc: '列出所有可用标签' },
...
function softMatchQaIntent(text) {
  ...
  // Person-specific queries: "小明的生日", "妈妈的纪念日", "XX的生日是什么时候"
  if (/(.+)的生日/.test(t) || /(.+)生日.*(什么时候|哪天|几号|快到了|还有几天)/.test(t) || /生日.*(.+)/.test(t)) {
    let name = '';
    const m1 = t.match(/(.+)的生日/);
    const m2 = t.match(/(.+)生日.*(什么时候|哪天|几号|快到了|还有几天)/);
    const m3 = t.match(/生日.*(.+)/);
    name = (m1?.[1] || m2?.[1] || m3?.[1] || '').replace(/^(帮我查|查|问|看|那)/g, '').trim();
    if (name && name.length <= 20 && !/^(谁|哪些|最近|什么|怎么)/.test(name)) {
      return { intent: 'person_query', name, via: 'soft' };
    }
  }
  ...
  // Anniversaries as a category
  if (/(纪念日|纪念).*(有哪些|最近|快到|即将|马上)/.test(t)
    || /有哪些.*纪念/.test(t)
    || /^(纪念日)$/.test(t)) {
    return { intent: 'anniversaries', via: 'soft' };
  }

  // Tag search: "查标签健康", "前端有哪些事项", "系统组件相关的"
  if (/(?:查|搜|找).*标签[：:\s]*(\S+)/.test(t)) {
    const m = t.match(/(?:查|搜|找).*标签[：:\s]*(\S+)/);
    const tag = (m?.[1] || '').replace(/[的之]/g, '').trim();
    if (tag && tag.length <= 20) return { intent: 'search_by_tag', tag, via: 'soft' };
  }
  if (/(\S+).*(?:相关|有关|关联).*(?:事项|习惯|事件|东西)/.test(t)) {
    const m = t.match(/(\S+).*(?:相关|有关|关联).*(?:事项|习惯|事件|东西)/);
    const tag = (m?.[1] || '').replace(/[的之]/g, '').trim();
    if (tag && tag.length <= 20 && !/^(什么|哪些|所有|全部|最近|今天)/.test(tag)) {
      return { intent: 'search_by_tag', tag, via: 'soft' };
    }
  }
  if (/(?:有哪些|列出|显示).*(?:标签|分类|类别|tag)/.test(t) || /^(标签|分类|tag|tags)$/i.test(t)) {
    return { intent: 'list_tags', via: 'soft' };
  }
